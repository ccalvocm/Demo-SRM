...
def DEVELOP_SRM(root, Basin, type_, Tcrit, alpha, beta, Dd_F, Rcs_F, Rcr_F, Tl_F,  X_F, Y_F):
    ...
    # leer time lags
    ruta_timelags = root+r'/timeLag.csv'
    time_lags = pd.read_csv(ruta_timelags).round()
    time_lags = time_lags.values.astype(int)
    # guardar time lags
    tls = time_lags[0,1]
    tlr = time_lags[0,0]
    ...
    lastDate = pd.read_csv(root+'\\LastDateVal.csv', index_col = 0).index[-1]
    
    # leer la ultima nieve acumulada
    sisim = pd.read_csv(root+r'/'+Basin+'_snowAcc.csv', header = None, index_col = 0)
        
    # leer los coeficientes de recesion simulados
    ksim = pd.read_csv(root+r'/'+Basin+'_k.csv', header = None, index_col = 0)
    ...
    if type_ == 'V':
        ...
        # leer flujo base
        BaseFlow = master['Bf'].dropna().values # Actual flow (m3/s)
    
        # inicializar el futuro baseflow
        BaseFlow = np.append(BaseFlow,BaseFlow[-1]*np.ones(len(master)-len(BaseFlow)))
                        
        # Leer caudales de la simulacion anterior
        Qsim = pd.read_csv(root+'\\Qsim_'+Basin+'.csv', header = None, index_col = 0)/1000 
        Qsim = Qsim.loc[:FirstDay]
    ...
    Tbands =     master[[x for x in master.columns if 'T_z' in x]]+Tlapse[0]+Tl_F;            # T bands(degC)
    Tbands = Tbands.values
    DegDaySnow = master['DegDaySnow']/100           # Factor grado-d�a para nieve(m/degday)
    DegDaySnow = DegDaySnow.values*Dd_F
    ...
    Qtot[:FirstDay] = np.transpose(Qsim.values[:FirstDay])
    ...
    # inicializar las variables de estado de la simulación anterior        
    k[FirstDay] = ksim.iloc[-1].values
    snowAcc = sisim.copy().values
    baseflow_ = np.copy(BaseFlow)
    ...
    loopCython.loop(FirstDay-1, Days, nZones, PCR2MET, Pbands, Tbands, Tcrit, snowAcc, A, summerdays, Qnewsnow, RCsnow, DegDaySnow, SCA, Qsnow, Qrain, RCp, Qglacial, hipso_glaciar,RCg, DegDayGlacier,  GCA )
    ...
    tol = 1
    i = 1
    while (tol >= 1e-2) & (i < 1e3):
        
        # calcular el caudal total simulado
        loopQtotalCython.loopQtotal(FirstDay-1, Days, nZones, apPluv, k, X, Qtot, Y, baseflow_, tls, tlr, Qsnow, Qnewsnow, Qglacial, Qrain)
    ...
    k[FirstDay] = ksim.iloc[-1].values
    snowAcc = sisim.copy().values        
    baseflow_[:FirstDay] = BaseFlow[:FirstDay]
    ...
    if type_ == 'V':         
        ...
        plt.close('all') 
        cve = pd.read_csv(root+r'\\CVE.csv', skiprows = 1)
    ...
    plt.plot(Days,Qactual[-730:-365],'k-', linewidth = 2)
    plt.plot(Days,Qtot[-730:-365],'r-', linewidth = 2)
    ...
    plt.axis([Days[0],Days[-1],0,1.5*max(max(Qtot[-730:-365]),max(Qactual[-730:-365]))])
    plt.grid()
    ...
    plt.grid()
    ax1.plot(Days,Tmean[-730:-365],'r-', linewidth = 1)
    ylim_min = min(Tmean[-730:-365])
    ylim_max = max(Pmean[-730:-365])
    ax1.set_ylim(ylim_min, ylim_max)
    ax2 = ax1.twinx()
    ax2.bar(Days,Pmean[-730:-365], color = 'b', width = 15, bottom = 0, linewidth = 2)
    ...
    out = pd.DataFrame(Qtot*1000, index = dates)
    out.to_csv(root+'\\Qsim_'+Basin+'.csv', header = None) 
