...
def readSnow(ruta_n):
    ...
    for filename in os.listdir(ruta_n):
        if 'DailySnowCover' in filename:
            df_n = pd.concat([df_n, pd.read_csv(ruta_n+'\\'+filename, index_col = 0, parse_dates = True)])
    ...
    for filename in os.listdir(ruta_n):
        if 'DailyGlacialCover' in filename:
            df_g = pd.concat([df_g, pd.read_csv(ruta_n+'\\'+filename, index_col = 0, parse_dates = True)])
