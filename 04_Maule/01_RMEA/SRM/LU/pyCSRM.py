...
def DEVELOP_SRM(root, Basin, alpha, Tcrit, plots):
    ...
    if plots:
        ...
        plt.plot(Days,Qtot,'b-', linewidth = 2)
        plt.title('Caudal medido vs simulado: ' +str(first_year)+'-'+str(last_year), fontsize=fontsize)
        # plt.xlabel('Days')
        plt.ylabel('Caudal ($m^3/s$)', fontsize=fontsize)
        plt.legend(['Q medido','Q Simulado'], fontsize=fontsize)
        ...
        # SWe observado
        # swe_obs = pd.read_csv(r'C:\Users\ccalvo\OneDrive - ciren.cl\Of hidrica\AOHIA_ZC\Etapa 3\Scripts\outputs\GEE_downloads\Maipo\MEEM\Maipo_MEEM_bandas_ERA5hourly_SWE_2000_2021_simplify_0_05.csv', index_col = 0, parse_dates = True)
        swe_obs = pd.read_csv(r'C:\Users\ccalvo\OneDrive - ciren.cl\Of hidrica\AOHIA_ZC\Etapa 3\Scripts\outputs\GEE_downloads\Maule\Maule_RMEA_bandas_ERA5hourly_SWE_2000_2021_simplify_0_05.csv', index_col = 0, parse_dates = True)        
        ...
        swe_obs_day = pd.DataFrame(swe_obs_day, index = dates, columns = ['SWE ERA5']) # SWE en m
        
        # plots
        fig, ax = plt.subplots(1, figsize=(18 , 3))
        swe_sim.plot(ax = ax, color = 'b')   
        swe_obs_day.plot(ax = ax, color = 'k')        
        plt.ylabel('Equivalente en agua de nieve (m)', fontsize = 14)
        plt.legend(prop={'size': 16}, loc = 'upper right')
        plt.grid()
        plt.xticks(Days_xticks[::frequency], Days_xticks[::frequency], rotation=rot)  # Set text labels and properties.
        ax.tick_params(axis='x', labelsize=16)
        ax.tick_params(axis='y', labelsize=13)
        plt.savefig("swe.png",bbox_inches='tight')
