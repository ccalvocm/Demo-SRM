...
call git pull
CD ..
call mshta "about:<script>alert('Chocolatey & miniconda instalados');close()</script>"
) else (
call git clone https://github.com/ccalvocm/Demo-SRM.git
call mshta "about:<script>alert('SRM descargado');close()</script>"
)
...
call mshta "about:<script>alert('Anaconda configurado');close()</script>"